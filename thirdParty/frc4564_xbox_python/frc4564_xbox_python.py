...
class Joystick:
    ...
    def __init__(self,refreshRate = 30):
        ...
        while waitTime > time.time() and not found:
            ...
            if readable:
                ...
                if response[0:7] == b'No Xbox':
                    raise IOError('No Xbox controller/receiver found. You may try `sudo rmmod xpad`')
        ...
        if not found:
            self.close()
            raise IOError('Unable to detect Xbox controller/receiver - Run python as sudo. You may try `sudo rmmod xpad`')
